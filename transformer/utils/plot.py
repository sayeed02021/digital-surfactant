...
def hist_box_list(data_list, name="hist_box", path="./", title=None,x_label=None,y_label=None):
...
def scatter_hist(x, y, name, path, field=None, lims=None):
    ...
    if "delta" in field:
        if "data" in field:
            axs[0].set_xlabel(r'$\Delta pCMC$ (experimental)')
            axs[0].set_ylabel(r'$\Delta pCMC$ (calculated)')
            xlabel = 'Delta pCMC (experimental)'
            ylabel = 'Delta pCMC (calculated)'
        elif "predict" in field:
            axs[0].set_xlabel(r'$\Delta pCMC$ (desirable)')
            axs[0].set_ylabel(r'$\Delta pCMC$ (generated)')
            xlabel = 'Delta pCMC (desirable)'
            ylabel = 'Delta pCMC (generated)'
    if "single" in field:
        if "data" in field:
            xlabel, ylabel = 'pCMC (experimental)', 'pCMC (calculated)'
            axs[0].set_xlabel(xlabel)
            axs[0].set_ylabel(ylabel)
        elif "predict" in field:
            xlabel, ylabel = 'pCMC (desirable)', 'pCMC (generated)'
            axs[0].set_xlabel(xlabel)
            axs[0].set_ylabel(ylabel)
